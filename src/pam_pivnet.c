...
static int run(const char *user,const char *pin,const char *key,
	const char *host,int port,const char *ifc,const char *helper)
{
	...
	strncpy(req.host,host,CONFSIZE-1);
	strncpy(req.dev,ifc,CONFSIZE-1);
	req.port=port;
	...
	const char **argv)
{
	...
	r=run(user,pass,key,host,port,dev,helper);
